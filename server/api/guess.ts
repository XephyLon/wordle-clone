import { defineHandle, useBody, createError, useCookie, setCookie } from 'h3';
...
export default defineHandle(async (req, res) => {
    ...
    const word = await storage.getItem(day) || validWords[Math.floor(Math.random() * validWords.length)]
...
function generateHint(word: string, guess: string): string {
    ...
    return [...guess].map((letter, i) => {
        ...
        if (exact) return '2'
        if (sourceWord.includes(guess[i])) return '1'
